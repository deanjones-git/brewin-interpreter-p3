...
class FuncInfo:
  def __init__(self, name, params, start_ip, captured = {}):
    self.name = name
    self.params = params  # format is [[varname1,typename1],[varname2,typename2],...]
    self.start_ip = start_ip    # line number, zero-based
    self.captured = captured  # if function is lambda, should capture values
...
class FunctionManager:
  ...
  def _cache_function_parameters_and_return_type(self, tokenized_program):
    ...
    for line_num, line in enumerate(tokenized_program):
      if line:
        if line[0] == InterpreterBase.FUNC_DEF:
          # format:  func funcname self.p1:t1 p2:t2 p3:t3 ...
          func_name = line[1]
          params = [self._to_tuple(formal) for formal in line[2:-1]]
          func_info = FuncInfo(func_name, params, line_num + 1)  # function starts executing on line after funcdef
          self.func_cache[func_name] = func_info

        if line[0] == InterpreterBase.FUNC_DEF or line[0] == InterpreterBase.LAMBDA_DEF:
          return_type_stack.append(line[-1])

        if line[0] == InterpreterBase.ENDFUNC_DEF or line[0] == InterpreterBase.ENDLAMBDA_DEF:
      ...
      # once function/lambda scope ends, use enclosing scope
