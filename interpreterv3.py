...
class Interpreter(InterpreterBase):
  ...
  def run(self, program):
    ...
    self.ip = self._get_function_info(InterpreterBase.MAIN_FUNC).start_ip
    self.return_stack = []
  ...
  def _is_member_access(self, token):
    return re.search("^[^.]+\.[^.]+$", token)

  def _get_variable_value(self, token):
    if self._is_member_access(token):
      objname, memname = token.split('.')
      maybe_object = self.env_manager.get(objname)
      if maybe_object is None:
        return None
      if maybe_object.type() != Type.OBJECT:
        super().error(ErrorType.TYPE_ERROR,
        f"{maybe_object.type()} variable {objname} is not object",
        self.ip
        )
      return maybe_object.value().get(memname)
  ...
  def _assign(self, tokens):
   ...
   if self.trace_output:
    print(f"ENV: {self.env_manager.environment[-1]}")
   self._advance_to_next_statement()
  ...
  def _funccall(self, args):
    ...
    if args[0] == InterpreterBase.PRINT_DEF:
      ...
      # For dummy functions (uninitialized func variables), just ignore this call
      if self._get_function_info == 'default':
        self._advance_to_next_statement()
      # Otherwise, actually carry out function call
      else:
        self.return_stack.append(self.ip+1)
        self.ip = self._find_first_instruction(args[0])
        self._create_new_environment(args[0], args[1:])

  def _lambda(self, args):
    symbolic_name = FunctionManager.create_lambda_name(self.ip)
    params = [self.func_manager._to_tuple(arg) for arg in args[:-1]]
    captured = self.env_manager.get_captured_values()
    func_info = FuncInfo(symbolic_name, params, self.ip + 1, captured)
    value_type = Value(Type.FUNC, func_info)

    result_var = InterpreterBase.RESULT_DEF + self.type_to_result[Type.FUNC]
  ...
  def _create_new_environment(self, funcname, args):
    formal_params = self._get_function_info(funcname)
    ...
    # if the function is an object's method,
    # add an implicit "this" parameter
    if self._is_member_access(funcname):
      objname = funcname.split('.')[0]
      tmp_mappings[InterpreterBase.THIS_DEF] = self.env_manager.get(objname)

    # create a new environment for the target function
    # and add all captured variables, then parameters to the env
    self.env_manager.push()
    self.env_manager.import_mappings(formal_params.captured)
  ...
  def _setup_default_values(self):
    ...
    self.type_to_default[InterpreterBase.FUNC_DEF] = Value(Type.FUNC,'default')
  ...
  def _get_function_info(self, funcname):
    if self.func_manager.is_function(funcname):
      return self.func_manager.get_function_info(funcname)
    
    maybe_func = self._get_variable_value(funcname)
    if maybe_func and maybe_func.type() == Type.FUNC:
  ...
  def _get_value(self, token):
    ...
    func_info = self._get_function_info(token)
    if func_info is not None:
      return Value(Type.FUNC, func_info)

    # look in environments for variable
    maybe_variable = self._get_variable_value(token)
  ...
  def _set_value(self, varname, to_value_type):
    if self._is_member_access(varname):
      objname, memname = varname.split('.')
      maybe_object = self.env_manager.get(objname)
      if maybe_object is None:
        super().error(ErrorType.NAME_ERROR,f"Assignment of unknown variable {objname}", self.ip)
      if maybe_object.type() != Type.OBJECT:
        super().error(ErrorType.TYPE_ERROR,
        f"{maybe_object.type()} variable {objname} is not object",
        self.ip
        )
      maybe_object.value()[memname] = to_value_type
    else:
  ...
  # bind the result[s,i,b,o,f] variable in the calling function's scope to the proper Value object
