...
class EnvironmentManager:
  ...
  def __repr__(self):
    output = [var + ':' + str(value) for var, value in self.environment.items()].join('\n')
    return output
  ...
  # used to capture all variables/parameters from an enclosing function 
  # for lambdas; since we capture by value, we need to deep copy
  def get_captured_values(self):
    captured = {}
    nested_envs = self.environment[-1]
    for env in nested_envs:
      for symbol, value in env.items():
        captured[symbol] = copy.deepcopy(value)
    return captured
